...
parser.add_argument('--testimgpath', default='../tmpimage', help='image')
...
#parser.add_argument('--netG', default='netmodels/model/netG_streetview_1.pth', help="path to netG (to continue training)")
parser.add_argument('--netG', default='model/netG_streetview_1.pth', help="path to netG (to continue training)")
...
def handler_imgin(imgname):
    try:
        print("coloring... ",imgname)
        image_margin = 0
        transform = transforms.Compose([transforms.ToTensor()])

        png_image = utils.load_image(opt.testimgpath+"/"+imgname, opt.imageSize)
        ...
        #vutils.save_image(result_img[0],'public/result/%s.png' % (imgname))
        result_img[0,0,
             image_margin:image_margin+opt.ndf,
        ...
        vutils.save_image(result_img[0],'../public/result/%s.png' % (imgname))
        '''
        result_img[0,0] = fake.data[0,0]
        result_img[0,1] = fake.data[0,1]
        result_img[0,2] = fake.data[0,2]
        vutils.save_image(result_img[0],'../public/result/%s.png' % (imgname))
        print('coloring has been done <PATH : ../public/result/%s.png>' % (imgname))
        os.remove(opt.testimgpath+"/"+imgname)
    except:
        os.remove(opt.testimgpath+"/"+imgname)
...
while 1:
    print(time.ctime())
    imgList = []
    for (dirpath,dirnames,filenames) in walk(opt.testimgpath):
